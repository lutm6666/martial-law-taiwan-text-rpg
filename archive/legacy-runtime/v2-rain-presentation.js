...
'use strict';
var sceneArt={
 '林宅客廳':'assets/v2/rain-parlor.svg?v=1',
 '阿信房間':'assets/v2/rain-son-room.svg?v=1',
 '林宅後巷':'assets/v2/rain-alley.svg?v=1',
 '陳太太騎樓':'assets/v2/rain-neighbor.svg?v=1',
...
function addCaption(card,name){if(card.querySelector('.rain-scene-caption'))return;var cap=document.createElement('span');cap.className='rain-scene-caption';cap.textContent='1958・臺北｜'+name;card.appendChild(cap)}
function decorateScene(){var card=document.querySelector('#v2Rain .v2-scene');if(!card)return;var name=sceneName(),src=sceneArt[name];if(src&&!card.querySelector('.rain-scene-art')){var img=document.createElement('img');img.className='rain-scene-art';img.src=src;img.alt=name+'場景';var body=card.querySelector('.v2-scene-body');card.insertBefore(img,body||card.firstChild)}if(src||card.querySelector('.v2-art'))addCaption(card,name);
 var note=card.querySelector('.v2-note');if(note&&!note.classList.contains('rain-dialogue')){var speaker=speakerByScene[name]||'調查筆記';note.classList.add('rain-dialogue');note.setAttribute('data-speaker',lastAction?speaker:'調查筆記');note.setAttribute('data-mode',lastAction?'dialogue':'note');if(lastAction){var t=document.createElement('span');t.className='rain-action-title';t.textContent='調查：'+lastAction;note.insertBefore(t,note.firstChild)}}}
