...
function maybeRestore(){
  const title=document.getElementById('sceneTitle')?.textContent.trim();if(title==='一封沒有署名的信'){localStorage.removeItem(EXP_KEY);return}
  const s=exp();if(s?.active){if(s.ending)renderEnding(s.ending);else if(s.node)renderNode(s.node);return}offerExpansion()
}
window.addEventListener('DOMContentLoaded',()=>{
  const game=document.getElementById('gameScreen'),saveBtn=document.getElementById('saveBtn'),restart=document.getElementById('restartBtn'),again=document.getElementById('againBtn');
  ...
  saveBtn?.addEventListener('click',e=>{const s=exp();if(!s?.active)return;e.stopImmediatePropagation();e.preventDefault();saveExp(s);toast('擴充章節進度已自動保存')},true);
  ...
  restart?.addEventListener('click',clearIfStart);again?.addEventListener('click',()=>{localStorage.removeItem(EXP_KEY)});
  schedule();
});
})();