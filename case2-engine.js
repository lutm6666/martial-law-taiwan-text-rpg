...
var evidenceFeedback={
 types:{
  scope_too_broad:'這項紀錄只支持較窄的範圍，還不能把「可以留下」推成「可以公開、轉交或任意使用」。',
  wrong_time:'現在要核對的是當時說過什麼。較晚形成的說法或擔心，不能直接取代同期紀錄。',
  indirect_only:'這項資料能提供推理方向，但不是直接回答這句證詞的證據。',
  unrelated_issue:'這項紀錄處理的是另一個爭點，還沒有碰到目前這句話真正聲稱的內容。',
  absence_is_not_consent:'沒有留下反對紀錄，不等於已取得同意；先找能直接界定當時問題與回答的資料。'
 },
 defaults:{a2:'scope_too_broad',m1:'wrong_time',q1:'indirect_only'},
 statements:{
  a2:{
   margin_note:'頁邊註記只證明「不要真名」這個匿名要求存在；它沒有證明內容可以照原樣整理、公開或轉交。',
   recovered_pages:'找回原稿只能證明這三頁存在；頁面本身沒有回答秋月當時究竟同意阿川做什麼。'
  },
  m1:{
   margin_note:'「不要真名」和「住處不要寫」不是同一句要求。這張註記不能證明美惠記得的住處限制當天真的被說過。',
   recovered_pages:'原稿內容能讓你知道訪談談了什麼，但目前要核對的是「當天是否說過住處不要寫」；這需要更接近當時的紀錄。',
   question_card:'提問卡回答的是阿川當時問了什麼，與美惠是否把後來的擔心混進記憶不是同一個爭點。',
   no_proxy:'這份紀錄形成在後來，而且談的是「誰能替秋月答應」。它不能直接證明美惠當天聽見哪些話。'
  },
  q1:{
   margin_note:'匿名要求只能證明「不要真名」；它沒有界定「可以留下、整理」這個同意是怎麼被問出來的。',
   recovered_pages:'原稿存在不等於原稿上的每種用途都獲得同意；目前需要核對的是當時問題本身的範圍。',
   meihui_scrap:'美惠的同期筆記可以旁證「可留」，但它仍是旁觀者的簡記；這一刻要核對的是秋月回答的原始提問範圍。',
   no_proxy:'「不能替秋月答應」是在處理第三人閱讀的後續問題，沒有直接核對秋月當時對保留與整理的回答。',
   direct_scope:'這就是秋月現在的說法本身。用同一句現在的證詞去證明自己，還缺少當時留下的獨立紀錄。',
   indirect_id:'這項紀錄談的是匿名後仍可能被辨識的風險，不是在回答「當時允許保留到什麼程度」。',
   reuse_boundary:'這項紀錄界定的是未來再次使用時要重新詢問，並不能直接證明最初那次提問的文字範圍。'
  }
 }
};
...
function normalizeSave(v){if(!v||v.caseId!=='case2')return null;var p=case1Profile();v.version=1;v.name=typeof v.name==='string'&&v.name.trim()?v.name.trim():p.name;v.role=roles[v.role]?v.role:p.role;v.focus=typeof v.focus==='number'&&isFinite(v.focus)?Math.max(0,Math.min(MAX_FOCUS,Math.floor(v.focus))):MAX_FOCUS;var rawPressed=v.pressed&&typeof v.pressed==='object'&&!Array.isArray(v.pressed)?v.pressed:{};var rawSolved=v.solved&&typeof v.solved==='object'&&!Array.isArray(v.solved)?v.solved:{};var validPressed={},validSolved={};function copyPressed(w){w.statements.forEach(function(st){if(rawPressed[st.id])validPressed[st.id]=true})}copyPressed(witnesses[0]);if(rawSolved.a2&&validPressed.a1&&validPressed.a2)validSolved.a2=true;var ready=function(w){for(var i=0;i<w.statements.length;i++)if(!validPressed[w.statements[i].id])return false;return !!validSolved[w.required]};var maxWitness=0;if(ready(witnesses[0])){maxWitness=1;copyPressed(witnesses[1]);if(rawSolved.m1&&validPressed.m1&&validPressed.m2)validSolved.m1=true;if(ready(witnesses[1])){maxWitness=2;copyPressed(witnesses[2]);if(rawSolved.q1&&validPressed.q1&&validPressed.a1)validSolved.q1=true}}v.pressed=validPressed;v.solved=validSolved;var allowed=['margin_note','recovered_pages',roleClue(v.role)];if(v.pressed.a1)allowed.push('question_card');if(v.pressed.m2)allowed.push('meihui_scrap');if(v.pressed.m3)allowed.push('no_proxy');if(v.pressed.q1)allowed.push('direct_scope');if(v.pressed.q2)allowed.push('indirect_id');if(v.pressed.q3)allowed.push('reuse_boundary');v.evidence=[];allowed.forEach(function(id){if(evidence[id]&&v.evidence.indexOf(id)===-1)v.evidence.push(id)});v.people=['achuan','qiuyue'];if(v.pressed.a3)v.people.push('meihui');v.flags=v.flags&&typeof v.flags==='object'&&!Array.isArray(v.flags)?v.flags:{};v.flags.qContact=!!v.pressed.m3;var requestedWitness=typeof v.witnessIndex==='number'&&isFinite(v.witnessIndex)?Math.max(0,Math.min(witnesses.length-1,Math.floor(v.witnessIndex))):0;v.witnessIndex=Math.min(requestedWitness,maxWitness);v.flags.finalReady=maxWitness===2&&ready(witnesses[2]);v.finalStep=typeof v.finalStep==='number'&&isFinite(v.finalStep)?Math.max(0,Math.min(finalQuestions.length,Math.floor(v.finalStep))):0;if(!v.flags.finalReady)v.finalStep=0;v.finished=!!v.flags.finalReady&&v.finalStep>=finalQuestions.length&&!!v.finished;v.failed=!v.finished&&v.focus<=0;v.feedback=typeof v.feedback==='string'?v.feedback:'';v.history=Array.isArray(v.history)?v.history:[];return v}
...
function showRoot(){mount();hideBase();$('case2Screen').classList.remove('hidden');$('caseChip').textContent='CASE 02・秋月的條件';var build=document.querySelector('.build');if(build)build.textContent='BUILD 5.1.6・CASE 02'}
...
function presentEvidence(eid){var id=pendingStatement,st=findStatement(id);$('c2Modal').classList.add('hidden');pendingStatement=null;if(!st)return;if(!st.correct){loseFocus('這一句目前不需要用證物核對。先判斷它是在提供資訊、還是在提出可被證物檢驗的主張。');return}if(st.correct!==eid){loseFocus(wrongEvidenceFeedback(id,eid));return}s.solved[id]=true;s.feedback=st.success;s.history.push({type:'present',statement:id,evidence:eid,result:'correct'});save();render();}
