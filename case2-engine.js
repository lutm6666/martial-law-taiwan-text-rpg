...
function load(){var v=parse(CASE2_KEY);return v&&v.caseId==='case2'?v:null}
...
function showRoot(){mount();hideBase();$('case2Screen').classList.remove('hidden');$('caseChip').textContent='CASE 02・秋月的條件';var build=document.querySelector('.build');if(build)build.textContent='BUILD 5.0.2・CASE 02'}
...
function renderRecord(){var box=$('c2Record'),html='<p class="c2-panel-title">案件紀錄</p><article class="section-card card"><h3>證物與情報</h3><div class="c2-record-grid">';s.evidence.forEach(function(id){var e=evidence[id];if(e)html+='<div class="c2-record"><strong>'+esc(e.name)+'</strong><small>'+esc(e.desc)+'</small><span class="c2-type">'+esc(e.type)+'</span></div>'});html+='</div></article><article class="section-card card" style="margin-top:11px"><h3>目前知道的人</h3><div class="c2-record-grid">';s.people.forEach(function(id){var p=people[id];if(!p)return;var desc=p.desc;if(id==='qiuyue'&&s.flags.qContact)desc='美惠已替你轉達。秋月願意親自說明：她當時允許保留什麼，又沒有允許什麼。';html+='<div class="c2-record"><strong>'+esc(p.name)+'</strong><small>'+esc(desc)+'</small></div>'});html+='</div></article>';box.innerHTML=html;}
