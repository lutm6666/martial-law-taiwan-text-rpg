...
BUILD = '5.1'
text = re.sub(r'BUILD (?:4\.6|5\.0(?:\.\d+)?|5\.1)・[^<]+', f'BUILD {BUILD}・CASE 02', text, count=1)
text = re.sub(r'(偵測到執行錯誤：[^\n]*?。BUILD )(?:4\.6|5\.0(?:\.\d+)?|5\.1)', rf'\g<1>{BUILD}', text, count=1)
...
engine = re.sub(r"build\.textContent='BUILD (?:5\.0(?:\.\d+)?|5\.1)・CASE 02'", f"build.textContent='BUILD {BUILD}・CASE 02'", engine, count=1)
...
script_tag = '<script src="case2-engine.js?v=6"></script>'
